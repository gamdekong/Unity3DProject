...
public class csMissile : MonoBehaviour {
    ...
    void MissileControl3()
    {
        ...
        Quaternion NewRotation = Quaternion.AngleAxis(Time.deltaTime * Speed * 2.0f, Axis) * transform.rotation;
        transform.rotation = Quaternion.Lerp(transform.rotation, NewRotation,  180.0f * Time.deltaTime);
        Pos = Vector3.forward * Time.deltaTime * Speed;           
